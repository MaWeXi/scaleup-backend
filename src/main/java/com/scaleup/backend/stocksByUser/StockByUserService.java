...
public class StockByUserService {
    ...
    public ResponseEntity<StockByUser> buyStock(StockBuy stockBuy) {
        ...
        try {
            ...
            if (stock.isPresent() && userByLeague.isPresent() && (userByLeague.get().getFree_budget()).intValue()>(stock.get().getPrice().multiply(BigDecimal.valueOf(amount))).intValue()){
                BigDecimal price = stock.get().getPrice();
                ...
                BigDecimal budget = userByLeagueUpdateBudget.getFree_budget();
                budget = budget.subtract(BigDecimal.valueOf(amount).multiply(price));
                userByLeagueUpdateBudget.setFree_budget(budget);
